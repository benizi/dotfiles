...
do {
	...
	my @buf;
	my $out = 0;
	thisfile: while (@buf or not $out) {
		while (@buf < 4 and not $out) {
			$^I = '' if $inplace;
			my $in = <>;
			if (defined $in) {
				debug "IN -> ".chrs($in)."\n";
		...
		last thisfile if not @buf;
		my ($is_utf, @c) = getu(@buf);
		debug "getu -> ($is_utf, ".chrsa(@c).")\n";
